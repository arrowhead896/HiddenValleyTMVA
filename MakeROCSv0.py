...
def SignalBGCompare1D(InputDir, signal, alg, variable, range, logy, pt1, pt2, m1, m2, outputdir):
    '''Implementation of simple signal and background comparison'''
    print "Making 1D comparison: ",signal,alg,variable
    ...
    print histname
    hsig = GetHist1D(InputDir+"ntuple_"+fileid+"_single.root",    "JetTree", histname, range, weight+"*("+alg+"_flavor=="+flavorid+")*mc_weight")
    hbkg = GetHist1D(InputDir+"ntuple_dijet_single.root", "JetTree", histname, range, weight+"*("+alg+"_flavor==0)*mc_weight")
    ...
    c.cd()
    rocSB.Draw("AC*")
    c.SaveAs(outputdir+"ROCDraw_"+alg+"_"+variable.replace("\\","")+"_pt"+pt1+pt2+"_"+signal+".eps")
    ...
    c.SetLogy(logy)
    c.SaveAs(outputdir+"SignalBGCompare_"+alg+"_"+variable+"_pt"+pt1+pt2+"_"+signal+".eps")

def GetMassEffs(InputDir, signal, alg, m1, m2, outputdir):
...
def SignalBGCompare2D(InputDir, alg, var1, var1cutdir, var2, var2cutdir, range1, range2, pt1, pt2, m1, m2, outputdir):
    ...
    #Get signal and background histograms
    hsig = GetHist2D(InputDir+"ntuple_ww_single.root",    "JetTree", alg+"_"+var1, alg+"_"+var2, range1, range2, weight+"*("+alg+"_flavor==1)")
    hbkg = GetHist2D(InputDir+"ntuple_dijet_single.root", "JetTree", alg+"_"+var1, alg+"_"+var2, range1, range2, weight+"*("+alg+"_flavor==0)")
...
def GetCorrelationAndSeparationSigBG(InputDir, alg, var1, var2, range1, range2, pt1, pt2, m1, m2, outputdir):
    ...
    #Get signal and background histograms
    hsig = GetHist2D(InputDir+"ntuple_ww_single.root",    "JetTree", alg+"_"+var1, alg+"_"+var2, range1, range2, weight+"*("+alg+"_flavor==1)")
    hbkg = GetHist2D(InputDir+"ntuple_dijet_single.root", "JetTree", alg+"_"+var1, alg+"_"+var2, range1, range2, weight+"*("+alg+"_flavor==1)")
...
for alg in algs:
    ...
    for CutRegion in CutRegions:
        ...
        if signal=="t":
            if CutRegion=="1": pt1="350"; pt2="500";   m1="140"; m2="200";
            if CutRegion=="2": pt1="600"; pt2="1000";  m1="140"; m2="200";
        if signal=="w" or signal=="z":
            if CutRegion=="1": pt1="350"; pt2="500";   m1="60"; m2="100";
            if CutRegion=="2": pt1="600"; pt2="1000";  m1="60"; m2="100";
        if signal=="h":
            if CutRegion=="1": pt1="350"; pt2="500";   m1="100"; m2="150";
            if CutRegion=="2": pt1="600"; pt2="1000";  m1="100"; m2="150";
        ...
        ##################################################################
        SignalBGCompare1D(InputDir, signal, alg, "m", "100,0,200", 0, pt1, pt2, "0", "200", outputdir1)        
        effsig,effsig_err,effbkg,effbkg_err = GetMassEffs(InputDir, signal, alg, m1, m2, outputdir1)
        ...
        if flag_singlevariable:
            for var in VarsAndRanges.keys():        
                 SignalBGCompare1D(InputDir, signal, alg, var, VarsAndRanges[var][int(CutRegion)], VarsAndRanges[var][0], pt1, pt2, m1, m2, outputdir1)    
